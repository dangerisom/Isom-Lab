...
import csv
import sys
from typing import List, Tuple, Optional  # keep if you plan to use type hints


class ImagePixelOverlapApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Vesicle Co-localization")
        ...
        # Yellow contours (BGR)
        self.contour_color_channel_1 = (0, 255, 255)
        self.contour_color_channel_2 = (0, 255, 255)
        ...
        self.btn_preview.grid(
            row=0, column=6, rowspan=2, padx=10, pady=5, sticky="nsew"
        )
        ...
        # Base save directory (user-chosen or derived from ch1 folder)
        self.save_base_dir = None
        # Actual output folder for the current run
        self.save_dir = None
    ...
    def short_path(self, path: str) -> str:
        ...
        if len(parts) >= 2:
            return f"{parts[-2]}{os.sep}{parts[-1]}"
        return parts[-1]
    ...
    def load_channel1(self):
        ...
        if path:
            ...
            self.save_base_dir = ch1_dir
            self.save_dir = None   # will be created per run
            self.channel2_path = None

            self.lbl_channel1.config(
                text=self.short_path(self.channel1_path),
                foreground="green"
            )
            self.lbl_channel2.config(
                text=f"{self.short_path(ch1_dir)}{os.sep}",
                foreground="green"
            )
            self.lbl_save.config(
                text=self.short_path(self.save_base_dir),
                foreground="green"
            )
    ...
    def load_channel2(self):
        ...
        if path:
            ...
            self.lbl_channel2.config(
                text=self.short_path(self.channel2_path),
                foreground="green"
            )
    ...
    def on_preview_threshold(self):
        ...
        if not self.channel1_path or not self.channel2_path:
            messagebox.showwarning(
                "Missing Channels",
                "Please load both image channels before previewing."
            )
            return

        out_dir = self.save_base_dir if self.save_base_dir else os.getcwd()
        ...
        try:
            ...
            # Thresholds
            t1_min = int(self.entry_t1_min.get())
            t1_int = int(self.entry_t1_int.get())
            t1_max = int(self.entry_t1_max.get())
            t2_min = int(self.entry_t2_min.get())
            t2_int = int(self.entry_t2_int.get())
            t2_max = int(self.entry_t2_max.get())
        ...
        self.threshold_and_contours(
            self.channel1_path, self.contour_color_channel_1, out_dir,
            t1_min, t1_int, t1_max, return_mask=False, display=True, save=False
        )
        self.threshold_and_contours(
            self.channel2_path, self.contour_color_channel_2, out_dir,
    ...
    def on_threshold_and_contours(self):
        ...
        # Create output folder only once per base dir change
        if not self.save_dir:
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            self.save_dir = os.path.join(self.save_base_dir, f"output_{timestamp}")
            os.makedirs(self.save_dir, exist_ok=True)

        self.lbl_save.config(
            text=self.short_path(self.save_dir),
            foreground="green"
        )
    ...
    def threshold_and_contours(
        self, path, contour_color, output_dir,
        t_min, t_int, t_max,
        ...
        for lo, hi, mask in [(t_min, t_int, mask_low),
            ...
            for cnt in contours:
                ...
                # outlines
                cv2.drawContours(
                    overlay, [cnt], -1, contour_color, self.contour_width
                )
                cv2.drawContours(
                    cont_only, [cnt], -1, contour_color, self.contour_width
                )
                # fill mask
